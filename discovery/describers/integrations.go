...
func GetIntegration(ctx context.Context, handler *resilientbridge.ResilientBridge, resourceID string) (*models.Resource, error) {
	...
	for _, syncObject := range integration.Syncs {
		syncs = append(syncs, provider.Sync{
	...
	value := models.Resource{
		...
		Description: provider.IntegrationDescription{
...
func processIntegrations(ctx context.Context, handler *resilientbridge.ResilientBridge, dopplerChan chan<- models.Resource, wg *sync.WaitGroup) error {
	var integrationListResponse provider.IntegrationListResponse
	...
	for _, integration := range integrationListResponse.Integrations {
		wg.Add(1)
		go func(integration provider.IntegrationJSON) {
			defer wg.Done()
			var syncs []provider.Sync
			for _, syncObject := range integration.Syncs {
				syncs = append(syncs, provider.Sync{
...
func processIntegration(ctx context.Context, handler *resilientbridge.ResilientBridge, resourceID string) (*provider.IntegrationJSON, error) {
	var integrationGetResponse provider.IntegrationGetResponse
