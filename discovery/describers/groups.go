...
func GetGroup(ctx context.Context, handler *resilientbridge.ResilientBridge, resourceID string) (*models.Resource, error) {
	...
	defaultProjectRole := provider.DefaultProjectRole{
...
func processGroups(ctx context.Context, handler *resilientbridge.ResilientBridge, dopplerChan chan<- models.Resource, wg *sync.WaitGroup) error {
	...
	var groupListResponse provider.GroupListResponse
	...
	for _, group := range groups {
		wg.Add(1)
		go func(group provider.GroupJSON) {
			defer wg.Done()
			defaultProjectRole := provider.DefaultProjectRole{
				Identifier: group.DefaultProjectRole.Identifier,
			}
...
func processGroup(ctx context.Context, handler *resilientbridge.ResilientBridge, resourceID string) (*provider.GroupJSON, error) {
	var groupGetResponse provider.GroupGetResponse
