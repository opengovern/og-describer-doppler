...
func GetProjectRole(ctx context.Context, handler *resilientbridge.ResilientBridge, resourceID string) (*models.Resource, error) {
	...
	value := models.Resource{
		...
		Description: provider.ProjectRoleDescription{
			Name:         projectRole.Name,
...
func processProjectRoles(ctx context.Context, handler *resilientbridge.ResilientBridge, dopplerChan chan<- models.Resource, wg *sync.WaitGroup) error {
	var projectRoleListResponse provider.ProjectRoleListResponse
	...
	for _, role := range projectRoleListResponse.Roles {
		wg.Add(1)
		go func(role provider.ProjectRoleJSON) {
			defer wg.Done()
			value := models.Resource{
				ID:   role.Identifier,
				Name: role.Name,
...
func processProjectRole(ctx context.Context, handler *resilientbridge.ResilientBridge, resourceID string) (*provider.ProjectRoleJSON, error) {
	var projectRoleGetResponse provider.ProjectRoleGetResponse
